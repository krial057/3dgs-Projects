...
AKP_Parser_Insertion* AKP_parser_insert(AKP_Parser_Insertion* head, int position, int insType, char* functionName) {
  ...
  if(head == NULL) {
    ...
    if(current == NULL) {//Node is the last node in the list
...
int AKP_parse_wait(char* azCode, int currentParsePos, AKP_Parser_Insertion** parseInsertList) {
  ...
  if(azWait[i] != '\0') return currentParsePos; //Wait was not completly parsed
  i = currentParsePos + i;
...
int AKP_parse_return(char* azCode, int currentParsePos, AKP_Parser_Insertion** parseInsertList) {
  ...
  if(azReturn[i] != '\0') return currentParsePos; //Wait was not completly parsed
  i = currentParsePos + i;
...
int AKP_parse_include(char* azCode, int currentParsePos, AKP_Parser_Insertion** parseInsertList) {
  ...
  if(azInclude[i] != '\0') return currentParsePos; //Wait was not completly parsed
  i = currentParsePos + i;
  i = AKP_parse_whitespace(azCode, i);
  ...
  if(strcmp(includeFileName, "AKProfiler.c") != 0) { //TODO do not parse files that were already parsed!
...
int AKP_parse_function_body(char* azCode, int currentParsePos, AKP_Parser_Insertion** parseInsertList) { //Parses backwards
...
void AKP_parse_lite_c(char* azCode, char* resultScript) {
  ...
  while(azCode[parsePos] != '\0') {
    int nextParsePos;
    parsePos = AKP_parse_whitespace(azCode, parsePos); //Skip whitespace
    nextParsePos = AKP_parse_wait(azCode, parsePos, &parseInsertList);
    if(nextParsePos == parsePos) { //No return found
    ...
    if(nextParsePos == parsePos) { //No wait found
...
void AKP_parse_file(char* sourceFileName) {
	...
	char *fileContent = sys_malloc(MAX_SCRIPT_SIZE);
	var fileHandleSrc = file_open_read(sourceFileName);
	file_str_readto(fileHandleSrc,fileContent, "", MAX_SCRIPT_SIZE);
	file_close(fileHandleSrc);
	
	char* resultContent = sys_malloc(MAX_SCRIPT_SIZE);
...
void AKP_init_startup() {
	char sourceFileName[260]; //260 = max. recommanded file name length windows
	int i;
